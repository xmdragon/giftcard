...
class EnhancedGiftCardApp {
    ...
    bindEvents() {
        ...
        if (performCheckin) {
            performCheckin.addEventListener('click', () => {
                this.performCheckin();
            });
        }
        ...
        // 返回按钮
        const backToGiftCard = document.getElementById('backToGiftCard');
        if (backToGiftCard) {
            backToGiftCard.addEventListener('click', () => {
    ...
    showError(message) {
        ...
        if (!statusDiv) {
            ...
            statusDiv.className = 'fixed top-0 left-0 w-full z-50';
    ...
    // 其他方法将在后续添加...
    async showGiftCardPage(giftCardCode) {
        this.showPage('giftCardPage');
        // 实现礼品卡页面逻辑
    }
